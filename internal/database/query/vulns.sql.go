...
const retrieveVulnTableByScan = `-- name: RetrieveVulnTableByScan :many
WITH root_account AS (
    SELECT COALESCE(
            (
                SELECT root_account_id
...
type RetrieveVulnTableByScanParams struct {
	AccountID pgtype.UUID
	ScanID    pgtype.UUID
}

type RetrieveVulnTableByScanRow struct {
	ID             pgtype.UUID
...
func (q *Queries) RetrieveVulnTableByScan(ctx context.Context, arg RetrieveVulnTableByScanParams) ([]RetrieveVulnTableByScanRow, error) {
	rows, err := q.db.Query(ctx, retrieveVulnTableByScan, arg.AccountID, arg.ScanID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []RetrieveVulnTableByScanRow
	for rows.Next() {
		var i RetrieveVulnTableByScanRow
		if err := rows.Scan(
			&i.ID,
			&i.Vulnerability,
			&i.Severity,
