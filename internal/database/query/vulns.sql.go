...
func (q *Queries) RetrieveVulnTable(ctx context.Context, rootAccountID pgtype.UUID) ([]RetrieveVulnTableRow, error) {
	rows, err := q.db.Query(ctx, retrieveVulnTable, rootAccountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
