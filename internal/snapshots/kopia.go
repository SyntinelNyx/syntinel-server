...
func (h *Handler) CreateKopiaS3Repository() {
	...
	controlMessages := []*controlpb.ControlMessage{
		...
		{
			Command: "exec",
			Payload: fmt.Sprintf("kopia policy set --global --add-ignore /proc --add-ignore /dev --add-ignore /sys --add-ignore /run --add-ignore /mnt --add-ignore /media --add-ignore /lost+found --add-ignore /tmp --add-ignore /var/tmp --add-ignore /nix/store"),
		},
	}
